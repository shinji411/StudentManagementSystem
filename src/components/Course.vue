<template>
  <div>
      ...
      <table class="course-table">
        <thead>
          <tr>
            ...
            <th v-for="w in course.week" :key="w">
              星期{{ course.weekChinese[w - 1] }}
            </th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="l in course.lesson" :key="l">
            ...
            <td
              v-for="d in course.showList[l - 1]"
              :class="{ 'course-active': d.span != 1 }"
              :rowspan="d.span"
              :key="d.id"
            >
              <!-- prettier-ignore -->
    ...
    <el-table :data="course.detail" border style="width: 100%">
...
export default {
  ...
  methods: {
    putCourseInfo(week) {
      let list = getCourseTable(
        this.course.detail,
        this.course.lesson.length,
        week
      );
